...
# 预设音线配置（engine 由全局 voice.provider 决定）
VOICE_PRESETS = {
    "sweet_female": {
...
class TTSRouter:
    ...
    async def synthesize(
        ...
        # 确定引擎（全局设置优先，预设不再写死 provider）
        provider = settings.get("voice.provider", "edge")
        ...
        # 透传设置中可能需要的引擎参数（如 API key）
        extra_kwargs = {}
        if provider == "openai":
            extra_kwargs["api_key"] = settings.get("model.cloud.api_key", "")

        try:
            return await engine.synthesize(text, engine_voice_id, speed, pitch, **extra_kwargs)
