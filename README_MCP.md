...
MCP（Model Capability Protocol）是 Auralis 的能力插件协议，定义了 Agent 如何发现、调用、管理操作系统能力。

### 核心概念

```
MCP = Capability + Policy + Context + Execution Contract
```

- **Capability**：做什么（如 `file.delete`）
- **Policy**：安全策略（风险等级、是否需要确认）
- **Context**：执行环境（OS、用户、会话）
- **Execution**：谁来执行（插件名、方法名）
...
源码位置：[plugins/file.mcp/plugin.py](plugins/file.mcp/plugin.py)
...
源码位置：[plugins/app.mcp/plugin.py](plugins/app.mcp/plugin.py)
...
源码位置：[plugins/system.mcp/plugin.py](plugins/system.mcp/plugin.py)
...
源码位置：[agent/mcp/plugin_loader.py](agent/mcp/plugin_loader.py)
...
源码位置：[agent/policy/risk_engine.py](agent/policy/risk_engine.py)
...
源码位置：[agent/policy/permission.py](agent/policy/permission.py)
